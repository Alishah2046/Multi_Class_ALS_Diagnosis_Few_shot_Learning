...
def get_complexity(arrays: list, plot: bool = False) -> int:
    ...
    # Robustness: `compute_complexity_array` can emit NaN/inf when a signal has
    # zero variance (std=0) or contains invalid values. Replace with 0 so PCA
    # does not crash; this preserves the rest of the feature extraction.
    ...
    try:
        ...
        complexity_idx = int(np.where(cum_var > 0.95)[0][0])
